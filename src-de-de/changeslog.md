...
- Installerversion: `26.10.04`.
- Das Excel-Exportlayout im [BOM Manager](pdmpublishersolidworks_bom-manager.md) wurde verbessert. **Browse...** befindet sich jetzt direkt neben dem Feld **Workbook template**.
- Die Miniaturansichtsoption für den Excel-Export wurde in **Include thumbnail** umbenannt.
...
- Das Löschen von Spalten in [Property Doctor](pdmpublishersolidworks_property-doctor.md#edit-properties) wurde korrigiert, sodass mehrere ausgewählte Eigenschaftsspalten gemeinsam zum Löschen markiert oder wiederhergestellt werden können. Wird der Befehl auf einer nicht ausgewählten Spalte verwendet, ist weiterhin nur diese Spalte betroffen.
...
- Eine ERP Sync-Quellenauswahl für den aktiven [SOLIDWORKS-Feature tree, SOLIDWORKS-Stücklistentabellen und UTF-8-CSV-Dateien](pdmpublishersolidworks_erp-sync.md#choose-the-synchronization-source) wurde hinzugefügt.
- Quellen aus SOLIDWORKS-Stücklistentabellen verwenden sichtbare Zeilen und Spalten, angezeigte Mengen und die ausgewählte Tabellenkonfiguration. Gültige eingerückte Tabellen können auch die Stücklistenhierarchie liefern.
- CSV-Quellen stellen dem Konnektor jede Spalte bereit und können die Stücklistenhierarchie über erkannte Spalten für Itemcode und übergeordnetes Item liefern.

<a id="20260927"></a>
## 2026.09.27
*Gilt für das **SOLIDWORKS Add-in***

- Die [ERP Sync-Berichterstellung](pdmpublishersolidworks_erpnext-connector.md#synchronisierung-überwachen-und-bericht-prüfen) wurde korrigiert, sodass jedes aktivierte Item ein eindeutiges Ergebnis **Success**, **No sync needed** oder **Failure** erhält.
- Erfolgreiche Konnektoren ohne einzelne Item-Ergebnisse melden die betroffenen Zeilen jetzt als **Success** mit einer erklärenden Ersatzmeldung statt **Not reported**.
- Statusbasierte Zeilenfarben, klarere Fehlerdetails, Statussuche und ein übersichtlicheres Berichtsraster wurden hinzugefügt.
