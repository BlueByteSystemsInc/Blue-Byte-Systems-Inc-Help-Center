...
| `silent` | Utilisez `False`. Les opérations ERP Push et Pull actuelles exigent une révision; `True` s'arrête avant la connexion et retourne une erreur. |

ERP Push et Pull ne peuvent pas s'exécuter silencieusement, car PDMPublisher exige que l'utilisateur examine les différences avant toute modification à distance ou dans SOLIDWORKS.
